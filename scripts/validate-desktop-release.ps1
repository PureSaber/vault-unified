    ...
    foreach ($root in $registryRoots) {
        ...
        foreach ($key in Get-ChildItem -Path $root -ErrorAction SilentlyContinue) {
            ...
            if ($item.InstallLocation) {
                $candidates += (Join-Path ([string]$item.InstallLocation) "Vault Unified.exe")
            }
            if ($item.DisplayIcon) {
                $candidates += (([string]$item.DisplayIcon -replace ',\d+$', '').Trim('"'))
            }
            if ($item.UninstallString) {
                ...
                if ($uninstall.StartsWith('"')) {
                    ...
                    if ($quote -gt 1) {
                        $candidates += (Join-Path (Split-Path $uninstall.Substring(1, $quote - 1)) "Vault Unified.exe")
                    }
                }
            }
