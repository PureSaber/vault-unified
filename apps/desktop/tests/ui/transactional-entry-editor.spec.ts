...
async function addGeneratedEntry(page: Parameters<MockAuthenticatedSidecar["install"]>[0], title: string) {
  ...
  await page.getByLabel("Website or app name", { exact: true }).fill(title);
...
test("keeps custom-field focus stable and guards navigation, cancel, and manual lock", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Add password", exact: true }).click();
  await page.getByLabel("Website or app name", { exact: true }).fill("Generated unsaved draft");
  await page.getByRole("button", { name: "More options", exact: true }).click();
  await page.getByRole("button", { name: "Add custom field", exact: true }).click();
  const label = page.getByLabel("Field name", { exact: true });
  ...
  await expect(page.getByLabel("Website or app name", { exact: true })).toHaveValue("Generated unsaved draft");
...
test("a failed attachment batch leaves the persisted entry unchanged", async ({ page }) => {
  ...
  await page.getByLabel("Website or app name", { exact: true }).fill("Must not partially save");
  await page.getByRole("button", { name: "More options", exact: true }).click();
...
test("history restore stays in the draft and cancel preserves the current version", async ({ page }) => {
  ...
  await page.getByLabel("Website or app name", { exact: true }).fill(currentTitle);
  await page.getByRole("button", { name: "Save", exact: true }).click();
  ...
  await expect(page.getByLabel("Website or app name", { exact: true })).toHaveValue(originalTitle);
...
test("navigation is blocked during save and a double click commits only once", async ({ page }) => {
  ...
  await expect(page.getByLabel("Website or app name", { exact: true })).toHaveValue(title);
...
test("automatic lock warns, clears the in-memory draft, and writes nothing", async ({ page }) => {
  ...
  await createVault(page);
  await page.getByRole("button", { name: "Add password", exact: true }).click();
  await page.getByLabel("Website or app name", { exact: true }).fill("Generated auto-lock draft");
  await page.getByLabel("Password", { exact: true }).fill(testData.entryPassword);
