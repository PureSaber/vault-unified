...
Evidence reconciled on **2026-09-19** from GitHub release metadata, tag objects, CI #134 results, the published-source validator, and the existing manual acceptance and owner decision in issue #30. The published manifest alone was downloaded and its digest verified during this correction. No new Windows test run, installer rebuild, binary re-download or owner approval is claimed. No raw credential-bearing logs are copied here.

**Status: published; owner approval and release completion recorded.** The [release checklist](release-readiness-v1.3.md) distinguishes automated checks, manual acceptance and remaining coverage limitations.
...
`Launch-And-StopInstalledApp` checks that the installed process survives an eight-second launch window, then stops it. It does not perform create-use-lock through the installed renderer. The separate sidecar smoke must not be counted as that installed-UI journey. Packaged failed-restore byte preservation is not established by this validator.

Separate [manual acceptance](https://github.com/PureSaber/vault-unified/issues/30#issuecomment-5645806553) records installed NSIS create/use/lock/reopen, discard-and-close, backup wording and browser pairing/fill/refusal/lock-revocation. These product paths were exercised on `5e56948`; the final `76c610a` changes installer validation only. [Final installer acceptance](https://github.com/PureSaber/vault-unified/issues/30#issuecomment-5645771218) records clean NSIS/MSI lifecycles on the rebuilt candidate. At the owner's request, MSI-specific installation, payload, startup, exit and cleanup were checked without repeating the shared product journey. These were guided owner checks using generated data, not independent novice research or arbitrary-website coverage.
...
The manifest does not list itself, so that CI step did not independently hash it against the Release API's own digest. The [final release evidence](https://github.com/PureSaber/vault-unified/issues/30#issuecomment-5646056634) separately records a fresh local download and verification of all four assets on 2026-09-12. On 2026-09-19, a fresh download of the manifest alone again matched its 1,337-byte size and API-reported SHA-256 above. No new installer execution or binary re-download was performed during this correction.

Local `release-manifest-v*.json` files generated by the build validator describe that local candidate; they are not automatically the published manifest, even at the same version/source. Keep candidate manifests with their candidate artifacts and official downloads in a separate directory. Use the manifest downloaded from the release when checking published assets. The stale root candidate manifest was archived locally during this correction, not uploaded or substituted for the official one.

## Owner decision and remaining limitations

The [owner decision recorded on 2026-09-12](https://github.com/PureSaber/vault-unified/issues/30#issuecomment-5645979059) explicitly states that the real-novice results review passed and authorizes the tag. This decision predates publication; it is not inferred from the tag author or CI. Underlying participant results, sample size, percentages and SUS scores were not supplied to Codex and are not claimed. The guided owner walkthrough remains a separate evidence category.

The [final release evidence](https://github.com/PureSaber/vault-unified/issues/30#issuecomment-5646056634) records no open P0/P1 issue at closure; a separate earlier review timestamp is not reconstructed. Remaining boundaries include unsigned installers, missing explicit installer `kind` fields in schema 1, no separate MSI product journey, and no packaged failed-restore byte-preservation evidence in these records. UI axe assertions cover their tested screens, and RustSec informational warnings remain subject to the [risk register](rustsec-risk-register.md). These limitations do not reopen the completed release or imply that those product behaviors are broken. Future releases still require their own appropriate evidence and owner decision.
